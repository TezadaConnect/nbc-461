...
class RecordsListExport implements FromView, WithEvents
{
    ...
    public function view(): View {
        ...
        if ($this->level == "research")
            $table_format = GenerateTable::whereIn('type_id', [2,5])->whereIn('report_category_id', [1,2,3,4,5,6,7])->get();
        else 
        ...
        foreach ($table_format as $format) {
            if ($format->is_table == "" || $format->report_category_id == null)
                ...
                if ($this->level == "research"){
                    $table_contents[92] =
                        Research::where('has_new_commitment', 1)->whereYear('created_at', $this->year)
                        ->where('discipline', $this->clusterID)->select('research.*')
                        ->get()->toArray();
                    $table_contents[93] =
                        Research::where('status', 32)->whereYear('created_at', $this->year)
                        ->where('discipline', $this->clusterID)->select('research.*')
                        ->get()->toArray();
                    if ($format->id != 92 || $format->id != 93){
                        $table_contents[$format->id] =
                            Report::where('reports.research_cluster_id', $this->clusterID)
                                ->where('report_category_id', $format->report_category_id)
                                ->where('reports.format', 'f')
                                ->where('reports.researcher_approval', 1)
                                ->where('reports.report_year', $this->year)
                                ->select('reports.*')->where('reports.report_category_id', $format->report_category_id)
                                ->groupBy('report_reference_id')
                                ->get()->toArray();
                    }
                } else{
                    $table_contents[$format->id] =
        ...
        $level = $this->level;
        $year = $this->year;
        return view('reports.generate.list-records', compact('table_format', 'table_columns', 'table_contents', 'year', 'clusterName', 'level'));
